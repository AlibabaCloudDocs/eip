...
-   按量付费EIP FAQ
    -   [已经释放了按量付费EIP，为什么还会产生费用？](#section_1nh_ax9_e2l)
    -   [为什么要为按使用流量计费的按量付费EIP设置带宽峰值？](#section_am7_sfw_w5y)
-   EIP限制FAQ
...
## 已经释放了按量付费EIP，为什么还会产生费用？ {#section_1nh_ax9_e2l .section}

对于按量付费的EIP，账单是在下一个小时产生。您会在释放EIP后收到账单，但其实已经停止了计费。例如：

一个按使用流量计费的EIP实例，如果您在10：30释放EIP实例，那么您在释放后会收到10点到11点的流量费的账单。

## 为什么要为按使用流量计费的按量付费EIP设置带宽峰值？ {#section_am7_sfw_w5y .section}

对于按使用流量计费的按量付费EIP，可以通过设置带宽峰值避免产生过多的出方向流量，以达到控制成本的目的。

按使用流量计费的按量付费EIP的带宽峰值不提供任何服务质量承诺。例如一个按使用流量计费的EIP的带宽峰值设置为200Mbps，不承诺带宽峰值一定可以达到200Mbps。如果希望使用有服务质量承诺的带宽峰值，您需要购买按带宽计费的EIP或共享带宽。
...
-   每个辅助弹性网卡绑定一个EIP，再将辅助弹性网卡绑定到一个ECS实例。每种实例规格支持绑定的辅助弹性网卡数量不同，详细信息，请参见[实例规格族](../../../../intl.zh-CN/实例/实例规格族.md#)。
-   EIP以**普通模式**绑定辅助弹性网卡，此模式下，您可以将多个EIP分别绑定到一个辅助弹性网卡的辅助私网IP上，然后再将辅助弹性网卡绑定到ECS实例。详细信息，请参见[普通模式下网卡多EIP](../../../../intl.zh-CN/最佳实践/普通模式下网卡多EIP.md#)。
-   EIP以**多EIP网卡可见模式**绑定辅助弹性网卡，此模式下，您可以将多个EIP绑定到一个辅助弹性网卡，然后再将辅助弹性网卡绑定到ECS实例。详细信息，请参见[设置多EIP网卡可见模式](../../../../intl.zh-CN/用户指南/绑定云资源/绑定辅助弹性网卡/设置多EIP网卡可见模式.md#)。
