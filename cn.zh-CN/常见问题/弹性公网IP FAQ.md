...
-   预付费EIP FAQ
    -   [预付费EIP支持批量续费吗？](#section_owz_hcc_124)
    -   [预付费EIP支持转换为后付费EIP吗？](#section_bue_yuk_t6s)
    -   -   后付费EIP FAQ
    -   [已经释放了后付费EIP，为什么还会产生费用？](#section_1nh_ax9_e2l)
    -   [为什么要为后付费-按流量计费的EIP设置带宽峰值？](#section_am7_sfw_w5y)
    -   [后付费EIP支持按使用流量和按固定带宽互相转换吗？](#section_wba_mrz_6hb)
    -   [后付费EIP支持转换为预付费EIP吗？](#section_sar_kti_nbs)
-   EIP限制FAQ
...
-   绑定EIP FAQ
    ...
    -   [一个ECS实例仅支持绑定一个EIP，怎样实现一个ECS实例关联多个EIP？](#section_naa_kwx_eks)
...
## 预付费EIP支持批量续费吗？ {#section_owz_hcc_124 .section}

支持。您可以在[批量续费中心](https://renew.console.aliyun.com/center)进行批量续费。

## 预付费EIP支持转换为后付费EIP吗？ {#section_bue_yuk_t6s .section}

支持。

预付费EIP支持转换为后付费-按使用流量计费的EIP。详细信息，请参见[转后付费-按使用流量](../../../../cn.zh-CN/用户指南/管理预付费EIP/转后付费-按使用流量.md#)。

预付费EIP支持转换为后付费-按固定带宽计费的EIP。详细信息，请参见[转后付费-按固定带宽](../../../../cn.zh-CN/用户指南/管理预付费EIP/转后付费-按固定带宽.md#)。

## 已经释放了后付费EIP，为什么还会产生费用？ {#section_1nh_ax9_e2l .section}

对于后付费的EIP，账单是在下一个小时或下一天产生。您会在释放EIP后收到账单，但其实已经停止了计费。例如：

-   对于按使用流量计费的EIP实例，如果您在10：30释放EIP实例，那么您在释放后会收到10点到11点的流量费的账单。
-   对于按固定带宽计费的EIP实例，如果您在2019-01-01释放EIP实例，那么您在释放后会收到2019-01-01到2019-01-02的带宽费的账单。
...
## 后付费EIP支持按使用流量和按固定带宽互相转换吗？ {#section_wba_mrz_6hb .section}
...
详细信息，请参见[转后付费-按使用流量](../../../../cn.zh-CN/用户指南/管理后付费EIP/转后付费-按使用流量.md#)和[转后付费-按固定带宽](../../../../cn.zh-CN/用户指南/管理后付费EIP/转后付费-按固定带宽.md#)。

## 后付费EIP支持转换为预付费EIP吗？ {#section_sar_kti_nbs .section}

支持。

详细信息，请参见[转预付费-按固定带宽](../../../../cn.zh-CN/用户指南/管理后付费EIP/转预付费-按固定带宽.md#)。
...
-   EIP网卡可见模式
    ...
    此模式下，EIP替换辅助弹性网卡的私网IP，辅助弹性网卡将变为一个纯公网网卡，私网功能不再可用。EIP在操作系统内部的弹性网卡上可见，可直接通过ifconfig或ipconfig获取网卡上的公网IP地址。详细信息，请参见[设置EIP网卡可见模式](../../../../cn.zh-CN/用户指南/绑定云资源/绑定辅助弹性网卡/设置EIP网卡可见模式.md#)。

-   多EIP网卡可见模式

    此模式下，辅助弹性网卡的私网功能仍然可用。EIP在弹性网卡上可见，在操作系统配置静态IP后，可通过ifconfig或ipconfig获取网卡上的公网IP地址。详细信息，请参见[设置多EIP网卡可见模式](../../../../cn.zh-CN/用户指南/绑定云资源/绑定辅助弹性网卡/设置多EIP网卡可见模式.md#)。


## 一个ECS实例仅支持绑定一个EIP，怎样实现一个ECS实例关联多个EIP？ {#section_naa_kwx_eks .section}

您可以通过如下方法使ECS实例关联多个EIP：

-   每个辅助弹性网卡绑定一个EIP，再将辅助弹性网卡绑定到一个ECS实例。每种实例规格支持绑定的辅助弹性网卡数量不同，详细信息，请参见[实例规格族](../../../../cn.zh-CN/实例/实例规格族.md#)。
-   EIP以**普通模式**绑定辅助弹性网卡，此模式下，您可以将多个EIP分别绑定到一个辅助弹性网卡的辅助私网IP上，然后再将辅助弹性网卡绑定到ECS实例。详细信息，请参见[普通模式下网卡多EIP](../../../../cn.zh-CN/最佳实践/普通模式下网卡多EIP.md#)。
-   EIP以**多EIP网卡可见模式**绑定辅助弹性网卡，此模式下，您可以将多个EIP绑定到一个辅助弹性网卡，然后再将辅助弹性网卡绑定到ECS实例。详细信息，请参见[设置多EIP网卡可见模式](../../../../cn.zh-CN/用户指南/绑定云资源/绑定辅助弹性网卡/设置多EIP网卡可见模式.md#)。
